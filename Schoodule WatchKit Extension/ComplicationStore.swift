...
struct ComplicationStore {
    ...
    var periodNameProvider: CLKTextProvider {
        ...
        case .current:
            if period!.className.count > 5 {
                let shortIndex = period!.className.index(period!.className.startIndex, offsetBy: 5)
                return CLKSimpleTextProvider(text: period!.className, shortText: String(period!.className[..<shortIndex]))
            }
            return CLKSimpleTextProvider(text: period!.className)
        case .placeholder:
